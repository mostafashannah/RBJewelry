...
function parseDate(raw: string): Date {
  ...
  return new Date(y, m - 1, d, 12, 0, 0); // noon to avoid timezone issues
}

function mapCategory(cat: string, desc: string): string {
  const d = desc.toLowerCase();
  if (cat === "اعلانات" || d.includes("meta ads") || d.includes("اعلانات")) return "Ad Spend";
  if (cat === "خام") return "Materials";
  if (cat === "أعمال خارجية") return "Operations";
...
async function main() {
  ...
  console.log(`Total: ${total.toLocaleString()} EGP (should be 108,633)`);

  const byCategory: Record<string, number> = {};
  for (const e of expenses) {
    const cat = mapCategory(e.cat, e.desc);
